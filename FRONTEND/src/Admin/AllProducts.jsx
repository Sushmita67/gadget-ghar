          ...
          {products?.map((product) => (
            ...
            >
              <div className="h-52 sm:h-64 relative">
                {product.image?.url ? (
                  <img
                    src={product.image?.url?.startsWith('/uploads') ? `${IMAGE_BASE_URL}${product.image.url}` : product.image?.url || product.image}
                    alt={product.name}
                    className="rounded-t-lg w-full h-full object-cover"
                  />
                ) : (
                  <div className="rounded-t-lg w-full h-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
